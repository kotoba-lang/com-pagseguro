(ns pagseguro.main
  "Kotodama WASM entrypoint for the Pagseguro clean-room actor (L4) — Clojure port.
  ...
  facts (`pagseguro.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pagseguro")
(def tier "L4")
...
(def entity-specs
  [{:entity "Customer"       :plural "customers"       :id-prefix "pagsegur_cus"
   ...
   {:entity "Refund"         :plural "refunds"         :id-prefix "pagsegur_ref"
    :fields [:chargeId :amount :reason :status]       :required [:amount :reason]
    :coerce {:amount :int}                 :refs {:chargeId "Charge"}}
   {:entity "Payout"         :plural "payouts"         :id-prefix "pagsegur_pay"
    :fields [:amount :currency :arrivalDate :status]  :required [:amount :currency]
    :coerce {}                             :refs {}}
   {:entity "PaymentMethod"  :plural "paymentmethods"  :id-prefix "pagsegur_pay"
...
(defn emit-facts
  "EAVT facts for one record: {\"pagseguro.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pagseguro-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pagseguro-compat/ready)
